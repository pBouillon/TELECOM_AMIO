 ...
 */
public class IsTimeWithinInterval extends CurrentTimeCondition {
     ...
     */
    public IsTimeWithinInterval(long currentTimeInMillis, long startTimeInMillis, long endTimeInMillis) {
    ...
    public boolean isCurrentTimeBetweenBounds() {
        ...
        return (start.isBefore(current) && current.isBefore(end))
                || (end.isBefore(start)
                    && (start.isBefore(current) || current.isBefore(end)));
    }
